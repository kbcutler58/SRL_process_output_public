...
def main():
    ...
    parser.add_argument("--cid", dest="override_cid", required=False, help="Overrideable Project ID, e.g. 2e041c9b-eb38-4714-af57-e6dca8b4e7e2" )
    
    parser.add_argument("--run-path", dest="run_path", required=False, help="path within BSSH workgroup to run folder")
    parser.add_argument("--samples-path", dest="samples_path", required=False, help="path within project to samples CID folder")
    ...
    # Check to see if there are more than 100 total items in the projects list
    # If the total items is above 100, loop through to add extra projects to the project dictionary

    projects_dict2_total_items = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['total_item_count']
    logging.info(int(projects_dict2_total_items))

    if (args.override_cid):
        projects_dict = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['items']
        logging.info("Total Project Item Count Skipped")
    if (int(projects_dict2_total_items) > 100):
        logging.info(int(projects_dict2_total_items))
        projects_dict2_next_token = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['next_page_token']
        logging.info(projects_dict2_next_token)
        projects_dict2_last_token = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['last_page_token']
        logging.info(projects_dict2_last_token)
        projects_dict = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['items']
        if (projects_dict2_last_token):
            projects_dict2 = ICA_SDK.ProjectsApi(api_client).list_projects(page_token=f"{projects_dict2_last_token}").to_dict()['items']
            projects_dict.extend(projects_dict2)
        # if projects_dict2_next_token == projects_dict2_last_token:
        #     projects_dict = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['items']
        #     projects_dict2 = ICA_SDK.ProjectsApi(api_client).list_projects(page_token=f"{projects_dict2_last_page_token}").to_dict()['items']
        #     projects_dict.extend(projects_dict2)
    else:
        projects_dict = ICA_SDK.ProjectsApi(api_client).list_projects().to_dict()['items']

    logging.info(projects_dict)
    if (args.override_cid):
        cid = args.override_cid
        logging.info("Selected Override Project ID is"+f"{args.override_cid}")
    else:
    ...
    for run_id in args.run_id:
        ...
        if (args.samples_path):
            logging.info("samples path defined "+args.samples_path)
            #Get list of fastqs
            sample_path_start = args.samples_path.find("/runs")
            logging.info(sample_path_start)
            sample_path_no_volume = args.samples_path[sample_path_start:]
            logging.info(f'{sample_path_no_volume}*')
            fastq_dict=ICA_SDK.FilesApi(api_client).list_files(volume_name=[samplevolume], 
                                                      path=[f'{sample_path_no_volume}*'], 
                                                      recursive=True, 
                                                      page_size=1000, 
    ...
    if (args.run_path):
        logging.info("run path defined")
        run_path_start = args.run_path.find("/Runs")
        run_path_no_volume = args.run_path[run_path_start:]
        run_path_folder_only = args.run_path[run_path_start+6:-1]
        runfolder_dict=ICA_SDK.FoldersApi(api_client).list_folders(volume_name=[bsshvolume], 
                                                path=[f'{run_path_no_volume}/*'], 
    ...
    for run_id in args.run_id:
        # Find name of the folder using the Run ID, if run-path is provided use the name of the run-id

        # If run path is provided use run-id as the folder name (actual_bclrun_name)

        if (args.run_path):
            logging.info("run path defined")
            actual_bclrun_name = run_id
            bclrun_name = run_path_folder_only
        else:
    ...
    for run_id in args.run_id:
        ...
        if (args.samples_path):
            logging.info("samples path defined")
            sample_path_start = args.samples_path.find("/runs")
            sample_path_no_volume = args.samples_path[sample_path_start:]
            fastq_dict=ICA_SDK.FilesApi(api_client).list_files(volume_name=[samplevolume], 
                                                      path=[f'{sample_path_no_volume}*'], 
